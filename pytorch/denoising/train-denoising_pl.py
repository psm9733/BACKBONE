...
def main():
    model_name = backbone_name + "/lr=" + str(learning_rate) + "/wd=" + str(weight_decay) + "/batchsize=" + str(
        batch_size)
    max_epochs = 72
    ...
    model = DeNoising(feature_num=feature_num, input_shape=input_shape, batch_size=batch_size,
                      train_aug=train_transform, val_aug=valid_transform, workers=workers, learning_rate=learning_rate,
                      weight_decay=weight_decay)
    # model.to(device)
    checkpoint_callback = ModelCheckpoint(dirpath=save_dir, filename=model_name, monitor="PSNR_loss", mode='min',
                                          verbose=True, save_top_k=1)
    trainer = pl.Trainer(auto_lr_find=False, precision=16, max_epochs=max_epochs, gpus=1, accumulate_grad_batches=1,
