...
class ResNext12(nn.Module):
    def __init__(self, activation, in_channels, groups=32, bias=True):
        ...
        self.block1_end = ResidualBlock(in_channels=in_channels, activation=activation, out_channels=(64, 128, 256), kernel_size=(3, 3), stride=(1, 2, 1), padding='same', groups=groups, bias=bias)
        self.block2_end = ResidualBlock(in_channels=256, activation=activation, out_channels=(256, 256, 512), kernel_size=(3, 3), stride=(1, 2, 1), padding='same', groups=groups, bias=bias)
        self.block3_end = ResidualBlock(in_channels=512, activation=activation, out_channels=(512, 512, 1024), kernel_size=(3, 3), stride=(1, 2, 1), padding='same', groups=groups, bias=bias)
        self.block4_end = ResidualBlock(in_channels=1024, activation=activation, out_channels=(512, 512, self.output_channel), kernel_size=(3, 3), stride=(1, 2, 1), padding='same', groups=groups, bias=bias)
