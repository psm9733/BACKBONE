...
class RegNetX_200MF(nn.Module):
    def __init__(self, activation, in_channels, block_width, bottleneck_ratio, groups, padding='same', dilation=1, bias=True):
        ...
        self.output_channel = block_width[3]
        self.stage1 = RegNetStage(activation, 1, in_channels, block_width[0], bottleneck_ratio, groups, padding, dilation)
        self.stage2 = RegNetStage(activation, 1, block_width[0], block_width[1], bottleneck_ratio, groups, padding, dilation)
        self.stage3 = RegNetStage(activation, 4, block_width[1], block_width[2], bottleneck_ratio, groups, padding, dilation)
        self.stage4 = RegNetStage(activation, 7, block_width[2], block_width[3], bottleneck_ratio, groups, padding, dilation)
