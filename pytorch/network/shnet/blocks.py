...
class StemBlock(nn.Module):
    def __init__(self, in_channels, activation, out_channels, kernel_size, stride, mode = "in", padding = 'same', groups = 1, dilation=1, bias = True):
        ...
        self.conv2d_bn_1 = Conv2D_BN(in_channels, activation=activation, out_channels=out_channels[0], kernel_size=kernel_size, mode = mode, stride=stride[0], padding=padding, bias=bias)
        self.conv2d_bn_2 = Conv2D_BN(out_channels[0], activation=activation, out_channels=out_channels[1], kernel_size=kernel_size, mode = mode, stride=stride[1], padding=padding, bias=bias)
        if len(out_channels) == 3:
            self.conv2d_bn_3 = Conv2D_BN(out_channels[1], activation=activation, out_channels=out_channels[2], kernel_size=kernel_size, mode = mode, stride=stride[2], padding=padding, bias=bias)
